...
def gen_ex(mep=False):
    names = "ABCDEFGHIJKLMNOPQR"
    # number of attributes
    na = random.randint(3,4)
    # attribute values
    av = [[str(r) for r in range(random.randint(2,3))] for i in range(na)]
    # max tree depth
...
def mep(dtree, attr, pa=dict(), m=1, ca=False, lapl=False):
    ...
    def stat_est(st):
        ...
        for k, _ in st['classification#'].items():
            ...
            frm+=['p(%s=%s$|$current node) ='%(attr,k),f]
            i+=1
    ...
    def mepsub(dtree):
        ...
        def ms(st):
            ...
            if st['subtrees']==[]:
                ...
                if rev_est <= st['static estimate']:
                    rst['formulas']+=[["Reverse estimate $\\leq$ static estimate","CUT SUBTREES"]]
                else:
                    rst['formulas']+=[["reverse estimate $>$ static estimate","KEEP SUBTREES"]]
                # record trace
                r+=[rst]
                st['focus']='false'
                # prune if needed
...
def read_cut(use=None):
    ...
    if args.tree is None:
        if use is None:
            dtree,test,args.attribute = gen_ex(args.mep)
            attr = test.columns[-1]
            if args.mep:
                pa = dtree['pa']
        else:
    ...
    # REP input
    if not args.table:
        if args.rep and not args.tree is None:
    ...
    if args.mep:
        ...
        if args.no_preprocess:
            ...
            trace = mep(dtree, attr, pa=pa,ca=ca, lapl=args.laplace)
        else:
