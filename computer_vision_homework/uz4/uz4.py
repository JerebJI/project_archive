...
def harris(I,sigma=2,thr=0):
    ...
    C11 = conv2(G,G.T)(dx**2)
    C12 = conv2(G,G.T)(dx*dy)
    C22 = conv2(G,G.T)(dy**2)
    Cdet = C11*C22-C12**2
    Ctr = C11+C22
    r = Cdet - alpha*Ctr**2
    ps = np.argwhere(nonmaxima_suppression_box(r)>thr).T
    return r,ps
...
def e2a():
    ...
    _,hess1 = hessian_points(I1,0.009)
    _,hess2 = hessian_points(I2,0.009)
    desc1 = (simple_descriptors(I1,*(hess1)))
    desc2 = (simple_descriptors(I2,*(hess2)))
    corr = unzip(find_correspondences(desc1,desc2))
    lh1,lh2 = np.flip(np.array(list(zip(*hess1)))),np.flip(np.array(list(zip(*hess2))))
    display_matches(I1,lh1[corr[0]],I2,lh2[corr[1]])
e2a()

def find_matches(I1,I2,t,descript=simple_descriptors,dist=hell):
    _,hess1 = hessian_points(I1,t)
    _,hess2 = hessian_points(I2,t)
    """
    plt.clf()
    plt.subplot(1,2,1)
    ...
    #d2s=(descript(I2,*np.array(hess2).T[corr[1]].T))
    ##plt.clf()
    ##plt.plot(range(len(d1s)),sorted(([hamming(d1,d2) for d1,d2 in zip(d1s,d2s)])))
    #plt.show()
...
def e2b():
    I1 = gray(imread('data/graf/graf_a_small.jpg'))
    I2 = gray(imread('data/graf/graf_b_small.jpg'))
    lh1,lh2,corr = find_matches(I1,I2,0.001)
...
def genBRIEF(sampling='uniform',s=20,nd=128*6,sigma=2):
    ...
    match sampling:
        ...
        case 'gauss2':
            p=np.array([np.digitize(rng.normal(0,1/25*s**2/4,size=nd),np.arange(-s//2,s//2)),
                        np.digitize(rng.normal(0,1/25*s**2/4,size=nd),np.arange(-s//2,s//2))])-s//2
            q=np.array([np.digitize(rng.normal(0,1/100*s**2/4,size=nd),np.arange(-s//2,s//2)),
                        np.digitize(rng.normal(0,1/100*s**2/4,size=nd),np.arange(-s//2,s//2))])-s//2
            q = np.clip(p+q,-s//2,s//2)
    #case 'polar':
    #JK:W
    #3     p=pol2xy(np.digitize(rng.normal(0,1/25*s**2/4,size=nd),np.arange(-s//2,s//2)),
    #plt.clf()
    #plt.scatter(p[0],p[1])
    #plt.scatter(q[0],q[1])
    #plt.show()
...
def e3b(k=100,thr=3,perc=0.5):
    I1 = gray(imread('data/newyork/newyork_a.jpg'))
    I2 = gray(imread('data/newyork/newyork_b.jpg'))
    lh1,lh2,corr = find_matches(I1,I2,0.0045)
    #display_matches(I1,np.flip(lh1[corr[0]]),I2,np.flip(lh2[corr[1]]))
    mat1,mat2 = lh1[corr[0]],lh2[corr[1]]
    binl = []
    bmat = []
    best = None
    for i in range(k):
        ss = np.array(random.sample(range(len(corr[0])),4),dtype='uint')
        est = estimate_homography(mat2[ss],mat1[ss])
        inl=[]
        for s in ss:
            x = np.hstack([mat1[s],1]).reshape(3,1)
            y = np.hstack([mat2[s],1]).reshape(3,1)
            if l2(np.dot(est,y),x)<thr:
                inl+=[s]
        #print(mat1[ss],mat1[ss].shape)
        #print(mat1[inl],mat1[inl].shape)
        if len(inl)/len(ss)>perc:
            est = estimate_homography(mat2[inl],mat1[inl])
            inl1=[]
            for s in inl:
                x = np.hstack([mat1[s],1]).reshape(3,1)
                y = np.hstack([mat2[s],1]).reshape(3,1)
